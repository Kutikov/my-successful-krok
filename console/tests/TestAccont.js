class TestAccount {
    ...
    GetTestFailCounter(fails) {
        const contentL = document.getElementById('ratingCardTemplate').content.cloneNode(true);
        contentL.querySelector('.ratingTableFork').innerText = this.forkId.replace(/ø/g, ' ');
        contentL.querySelector('.ratingTableUnit').innerText = this.unitId.replace(/ø/g, ' ');
        contentL.querySelector('.ratingTableCount').innerText = fails.toString();
        this.DrawTestContents(contentL, -1);
        document.getElementById('ratingPanel').appendChild(contentL);
    }
